...
import ReceivePoints from "../../shared/ReceivePoints/receive_points"


function Practice(props) {

  const [isReceivePointsOpen, setIsReceivePoitsOpen] = useState(false)
  function openReceivePoints() {
    setIsReceivePoitsOpen(true)
  }

  const letters_to_sign = ['T', 'B', 'C', 'F', 'G', 'H', 'I', 'J', 'K', 'L', 'M']
      ...
      if (
        ...
        if (hand.length > 0) {
          ...
          if (estimatedGestures.gestures.length !== 0) {
            ...
            setPredictedLetter(maxValue.name)
  ...
  return (
    <div className="trypage">
            ...
            <div id='t' className={reviewText == "Detected!" ? "trypage_cam_content aquaborder" : "trypage_cam_content"}>
              {cameraActive && (
                <>
                  ...
                  />
                  <div className="preidciton" style={{color:'aqua', fontSize:'32px'}}>
                  {predictedLetter}
                  </div>
                  <canvas
                    className="can"
              ...
              <div className={cameraActive ? 'prompt' : 'prompt prompt1'}>
                {cameraActive && (
                  <div>
                    <img style={{ height: '80px', width: '100px' }} src={require(`../../../assets/images/sign_icons/${letters_to_sign[currentLetterIndex].toLowerCase()}.jpg`)} alt="Sign" />
                  </div>
                )}
                {!cameraActive && (
                  <div>
      ...
      {isReceivePointsOpen && (
        <ReceivePoints className="page2_create" user={props.user} points={points} />
      )}
    </div>
  );
